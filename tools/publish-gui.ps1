...
$Project = Join-Path $RepositoryRoot "src/DocRedock.Gui/DocRedock.Gui.csproj"
$OutputRoot = Join-Path $RepositoryRoot "artifacts/gui"
$SupportedRuntimeIds = @(
    "win-x64", "win-arm64",
    "osx-x64", "osx-arm64",
    "linux-x64", "linux-arm64"
...
foreach ($RuntimeId in $RuntimeIds) {
    ...
    Write-Host "Publishing DocRedock GUI for $RuntimeId"
...
Write-Host "Published GUI builds: $OutputRoot"
